...
import style from "./card_carousel.module.css";
...
const MyCardSlider = ({ product }) => {
  const { t, i18n } = useTranslation();
  const { handleDispatch, myRef } = useClickIcon(style, product);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  return (
    <div className={`${style.wrapper} container-fluid`} key={product?.id}>
...
export default memo(MyCardSlider);
